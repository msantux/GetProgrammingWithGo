...
const (
	...
	// The length of time per day during wich
	// messages can be transmitted from rover to Earth.
...
func NewRoverDriver(
	name string,
	occupier *Occupier,
	marstToEarth chan []Message,
) *RoverDriver {
	r := &RoverDriver{
		commandc: make(chan command),
		occupier: occupier,
		name:     name,
		radio:    NewRadio(marstToEarth),
	}
...
// Size return a Point representing the size od the grid.
func (g *MarsGrid) Size() image.Point {
	return g.bounds.Max
}

// Occupy occupies a cell at the given point in the grid. It
// returns nil if the point is already occupied or the poit is ouside
...
// It reports wheter the move was successful
